    ...
    public static void main (String args[]) throws Exception {
        try {
            ...
            if(!(serverReply.substring(0,4).equals("JOBN"))) {
                ...
                bout.flush();
                serverReply = csj.readMsg(new byte[124*124], bin);
            System.out.println("Received in response to OK: " +serverReply);
                bout.write(REDY.getBytes());
                bout.flush();
            ...
            if(message_space.length > 1) {
                
            serverReply = csj.readMsg(new byte[Integer.parseInt(message_space[1])*124], bin);
    
            }
            else {
                serverReply = csj.readMsg(new byte[10], bin);
